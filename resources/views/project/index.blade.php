...
<body>
    <div class="card">
        <div class="text-center text-bg-light">
            <h1 class="mt-2"> Project List </h1>
        </div>
        <div class="card-body">
            <!-- ONLY ADMIN and PROJECT MANAGER can see add new project button -->
            @if(Auth::user()->role_id != 3)
            <div class="addbtn">
                <a href="{{route('projectlist.create')}}" id="addnewsFeedBtn"
                    class="btn btn-primary waves-effect btn-label waves-light mr-1 bt-sm-newedit2 p-2"><i
                        class="bx bx-plus label-icon "></i>ADD NEW PROJECT</a>
            </div>
            @endif
            <div class="">
                <div class="mt-4">
...
function dataTableAjaxCall() {
    var dt = $('#projectList').DataTable({
        destroy: true,
        processing: true,
        responsive: true,
        autoWidth: false,
        ajax: {
            url: "{{ route('projectlist.index') }}",
        },
        columns: [{
                data: 'id',
                name: 'sequence',
